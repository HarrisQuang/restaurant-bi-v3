class Query:
    ...
    get_ranking_quantity_sales_dishes_vegan_day_tbl_with = {
        "single_day_with_xep_hang_sl_ban": "SELECT ngay_number, ngay_filter, ten_mon, sl_ban, xep_hang_sl_ban FROM quantity_sales_dishes_vegan_day where ngay_filter = '%s' and xep_hang_sl_ban <= %s order by xep_hang_sl_ban",
        "multi_days_with_xep_hang_sl_ban": "SELECT ngay_number, ngay_filter, ten_mon, sl_ban, xep_hang_sl_ban FROM quantity_sales_dishes_vegan_day where ngay_filter in %s and xep_hang_sl_ban <= %s order by xep_hang_sl_ban",
        "single_day_with_xep_hang_tong": "SELECT ngay_number, ngay_filter, ten_mon, tong, xep_hang_tong FROM quantity_sales_dishes_vegan_day where ngay_filter = '%s' and xep_hang_tong <= %s order by xep_hang_tong",
        "multi_days_with_xep_hang_tong": "SELECT ngay_number, ngay_filter, ten_mon, tong, xep_hang_tong FROM quantity_sales_dishes_vegan_day where ngay_filter in %s and xep_hang_tong <= %s order by xep_hang_tong"
    }